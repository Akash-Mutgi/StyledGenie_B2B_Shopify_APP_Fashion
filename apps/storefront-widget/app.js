...
const widgetLauncherButton = document.getElementById("widgetLauncherButton");
const preferencesConfirmationScreen = document.getElementById("preferencesConfirmationScreen");
...
const GUEST_SESSION_STORAGE_KEY = "styledgenie-storefront-guest-session-id-v1";
...
function normalizeApiBase(value) {
  return stripTrailingSlash(value).replace(/\/merchant-dashboard(?:\/index\.html)?$/i, "");
}
...
function getShopperIdentity() {
  ...
  const explicitEmail =
    (window.StyledGenieShopper && window.StyledGenieShopper.email) ||
    window.STYLEDGENIE_SHOPPER_EMAIL ||
    "";
...
function getOrCreateGuestSessionId() {
  ...
  if (!storage) {
    return `styledgenie-demo-${Date.now().toString(36)}`;
  }
  ...
  const generated =
    typeof crypto !== "undefined" && crypto && typeof crypto.randomUUID === "function"
      ? `styledgenie-demo-${crypto.randomUUID()}`
      : `styledgenie-demo-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`;
...
function trackWidgetAnalytics(eventName, payload = {}) {
  ...
  const detail = {
    event: eventName,
    surface: "storefront_widget_demo",
    mode: activeMode,
    ...payload,
  };
...
function createSuccessIcon(className = "preferences-success-icon") {
...
function createPrimaryButton(label, options = {}) {
  ...
  button.type = "button";
  button.className = options.className || "preferences-confirmation-primary";
...
function renderPreferenceConfirmationScreen() {
  ...
  card.className = "preferences-confirmation-card";

  const backButton = document.createElement("button");
  backButton.type = "button";
  backButton.className = "preferences-confirmation-back";
  ...
  copyBlock.className = "preferences-confirmation-copy";

  const title = document.createElement("h3");
  title.className = "preferences-confirmation-title";
  title.textContent = viewModel.title;

  const subtitle = document.createElement("p");
  subtitle.className = "preferences-confirmation-subtitle";
  subtitle.textContent = viewModel.subtitle;

  copyBlock.append(title, subtitle);

  const iconWrap = document.createElement("div");
  iconWrap.className = "preferences-confirmation-icon-wrap";
  iconWrap.appendChild(createSuccessIcon());

  const footer = document.createElement("div");
  footer.className = "preferences-confirmation-footer";

  if (viewModel.statusLabel) {
    const status = document.createElement("p");
    status.className = "preferences-confirmation-status";
  ...
  if (viewModel.errorMessage) {
    ...
    error.className = "preferences-confirmation-error";
  ...
  if (viewModel.showRetry) {
    ...
    retryButton.className = "preferences-confirmation-retry";
  ...
  const nextButton = createPrimaryButton(viewModel.buttonLabel, {
    className: "preferences-confirmation-primary",
...
function expandWidget() {
  ...
  widgetLauncherButton.setAttribute("aria-expanded", "true");
  scrollChatToBottom();
  if (chatInput && !isGuidedFeature(activeMode)) {
    chatInput.focus();
  }
}
...
function filterProductsForActiveSegment(products, shopperProfile, aiRuntime) {
  const draftProfileInputs = buildProfileInputsPayload();
  const requiredSegment =
    (draftProfileInputs && draftProfileInputs.segment) ||
...
async function sendTextChat(rawMessage, options = {}) {
  ...
  try {
    ...
    const shouldRetryWithoutProfileContext =
      (result.error || !result.response || !result.response.ok || !result.data) &&
      (requestPayload.profile_inputs || requestPayload.decision_mode !== null);
...
if (widgetLauncherButton) {
  widgetLauncherButton.addEventListener("click", () => {
    expandWidget();
  });
}
