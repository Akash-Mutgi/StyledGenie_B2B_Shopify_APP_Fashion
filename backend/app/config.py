...
def build_customer_account_profile_url() -> str:
    raw_domain = str(settings.shopify_storefront_domain or settings.shopify_store_domain or "").strip()
    if not raw_domain:
        return ""
    ...
    try:
        ...
        if host.endswith(".myshopify.com"):
            ...
            account_host = f"account.{root_host}"
            path = "/profile"

        return urlunparse((parsed.scheme or "https", account_host, path, "", "", ""))
    except Exception:
        return ""
