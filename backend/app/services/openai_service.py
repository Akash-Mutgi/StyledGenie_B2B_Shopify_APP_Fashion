...
from app.services.merchant_voice_service import MerchantVoiceService
...
class StyleProfileScanPlan(BaseModel):
...
class OpenAIService:
    def __init__(self) -> None:
        ...
        self.merchant_voice_service = MerchantVoiceService()
        self._merchant_context_cache: Optional[dict] = None
        self._merchant_context_cache_at: float = 0.0
        self._merchant_context_cache_ttl_seconds = 20.0
    ...
    def style_recommendations(
        ...
        voice_config = self._merchant_voice_config()
        ...
        try:
            response = self.client.responses.parse(
                ...
                input=[
                    {
                        "role": "system",
                        "content": self._system_instructions(voice_config=voice_config),
                    },
                    {
                        "role": "user",
        ...
        reply = self.merchant_voice_service.format_reply_text(
            (plan.reply or fallback_reply).strip(),
            voice_config,
        )
        ...
        if follow_up:
            reply = self.merchant_voice_service.format_reply_text(f"{reply} {follow_up}", voice_config)
                ...
                input=[
                    {
                        ...
                        "content": (
                            ...
                            f" {self.merchant_voice_service.build_voice_system_prompt(voice_config, audience='shopper-facing visual explainer')}"
                        ),
                    },
                    {
                        "role": "user",
    ...
    def assess_support_issue_image(
        ...
        *,
                ...
                input=[
                    {
                        ...
                        "content": (
                            ...
                            f" {self.merchant_voice_service.build_voice_system_prompt(voice_config, audience='shopper-facing support concierge')}"
                        ),
                    },
                    {
                        "role": "user",
        ...
        return {
            "reply": self.merchant_voice_service.format_reply_text(
                (parsed.reply or fallback["reply"]).strip() or fallback["reply"],
                voice_config,
            ),
            "summary": self.merchant_voice_service.format_reply_text(
                (parsed.summary or fallback["summary"]).strip() or fallback["summary"],
                voice_config,
            ),
            "confidence": (parsed.confidence or fallback["confidence"]).strip() or fallback["confidence"],
        }

    def analyze_style_profile_scan(
        self,
        *,
        vision_analysis: VisionAnalysis,
        draft_profile: Optional[StyleProfile] = None,
    ) -> StyleProfileScanResponse:
        voice_config = self._merchant_voice_config()
        fallback = self._fallback_style_profile_scan(
            vision_analysis=vision_analysis,
        ...
        try:
            response = self.client.responses.parse(
                ...
                input=[
                    {
                        "role": "system",
                        "content": (
                            "You are StyledGenie's profile scan assistant. "
                            "Review a shopper-uploaded full-body image and suggest only conservative, editable profile prefills. "
                            "You may suggest body type, skin tone or complexion, hair color, eye color, and broad style cues. "
                            "Never infer gender, sizes, identity, ethnicity, or sensitive personal attributes. "
                            "If confidence is low, leave the field blank and lower the confidence. "
                            "For body_type, use only: hourglass, pear, triangle, rectangle, apple, athletic, other, or blank. "
                            "Keep style_summary and style_description concise and useful for future styling. "
                            "Return short observation lines that help the shopper confirm what you saw. "
                            "This result must always remain editable and require confirmation."
                            f" {self.merchant_voice_service.build_voice_system_prompt(voice_config, audience='shopper-facing profile scan assistant')}"
                        ),
                    },
                    {
                        "role": "user",
                        "content": json.dumps(
                            {
                                "draft_profile": draft_profile.model_dump() if draft_profile else None,
                                "vision_analysis": {
                                    "summary": vision_analysis.summary,
                                    "labels": vision_analysis.labels,
                                    "objects": vision_analysis.objects,
                ...
                text_format=StyleProfileScanPlan,
            )
            parsed = response.output_parsed
        except Exception as error:
            logger.warning("OpenAI style profile scan analysis failed. %s", error)
            return fallback

        if not parsed:
            return fallback

        return StyleProfileScanResponse(
            suggestion=StyleProfileScanSuggestion(
                features=StyleProfileFeatures(
                    bodyType=self._normalize_style_profile_body_type(parsed.body_type or fallback.suggestion.features.bodyType),
                    skinTone=(parsed.skin_tone or fallback.suggestion.features.skinTone or "").strip() or None,
                    hairColor=(parsed.hair_color or fallback.suggestion.features.hairColor or "").strip() or None,
                    eyeColor=(parsed.eye_color or fallback.suggestion.features.eyeColor or "").strip() or None,
                ),
                vibe=StyleProfileVibe(
                    pinterestUrl=draft_profile.vibe.pinterestUrl if draft_profile and draft_profile.vibe else None,
                    imageUrls=draft_profile.vibe.imageUrls if draft_profile and draft_profile.vibe else [],
                    styleDescription=(parsed.style_description or fallback.suggestion.vibe.styleDescription or "").strip() or None,
                ),
                styleAnalysis=StyleProfileAnalysis(
                    summary=(parsed.style_summary or fallback.suggestion.styleAnalysis.summary).strip()
                    or fallback.suggestion.summary,
                    tags=self._dedupe_text(parsed.style_tags or fallback.suggestion.styleAnalysis.tags),
                ),
                source=StyleProfileSource(method="scan"),
                confidence=(parsed.confidence or fallback.suggestion.confidence or "medium").strip() or "medium",
                requiresConfirmation=True,
                qualityNote=(parsed.quality_note or fallback.suggestion.qualityNote or "").strip() or fallback.suggestion.qualityNote,
                observationLines=parsed.observation_lines or fallback.suggestion.observationLines,
                summary=(parsed.style_summary or fallback.suggestion.summary).strip() or fallback.suggestion.summary,
                fullLengthRecommended=fallback.suggestion.fullLengthRecommended,
            ),
            message=self.merchant_voice_service.format_reply_text(
                "Scan suggestions ready to confirm and adjust.",
                voice_config,
            ),
        )
    ...
    def compose_complete_look_reply(
        ...
        *,
                ...
                input=[
                    {
                        ...
                        "content": (
                            ...
                            f" {self.merchant_voice_service.build_voice_system_prompt(voice_config, audience='shopper-facing stylist')}"
                        ),
                    },
                    {
                        "role": "user",
        ...
        reply = self.merchant_voice_service.format_reply_text((parsed.reply or "").strip() or fallback_reply, voice_config)
                ...
                input=[
                    {
                        ...
                        "content": (
                            ...
                            "Never invent products or details. Return structured output only."
                            f" {self.merchant_voice_service.build_voice_system_prompt(voice_config, audience='shopper-facing inspiration stylist')}"
                        ),
                    },
                    {
                        "role": "user",
    ...
    def build_text_reply(
        ...
        if not recommendations:
            return self.merchant_voice_service.format_reply_text(
                ...
                ,
                voice_config or self._merchant_voice_config(),
            )
    ...
    def _fallback_style_profile_scan(
        self,
        *,
        vision_analysis: VisionAnalysis,
        draft_profile: Optional[StyleProfile] = None,
        ...
        style_description = ""
        if vision_analysis.style_cues:
            style_description = f"I’m reading {' and '.join(self._dedupe_text(vision_analysis.style_cues)[:2])} styling cues."
        elif vision_analysis.colors:
            style_description = f"I’m reading a {', '.join(vision_analysis.colors[:2])} colour direction."
        else:
            style_description = "I have a starter visual read, but I’d still want you to confirm and adjust it."

        summary = style_description
        body_type = None
        if any(cue in {"structured", "tailored", "athletic"} for cue in [tag.lower() for tag in vision_analysis.style_cues]):
            body_type = "athletic"

        confidence = "low" if vision_analysis.source == "fallback" else "medium"
        full_length_recommended = not any(
            str(item or "").lower() == "person" for item in (vision_analysis.objects or [])
        )
        quality_note = (
            "A full-body photo in steady light will make the body-shape cues more reliable."
            if full_length_recommended
            else "You can still edit anything that feels off before saving."
        )
        observation_lines = self._dedupe_text(
            vision_analysis.apparel_cues[:2]
            + vision_analysis.style_cues[:2]
            + vision_analysis.colors[:2]
        )[:4]

        return StyleProfileScanResponse(
            suggestion=StyleProfileScanSuggestion(
                features=StyleProfileFeatures(
                    bodyType=self._normalize_style_profile_body_type(body_type),
                    skinTone=None,
                    hairColor=None,
                    eyeColor=None,
                ),
                ...
                observationLines=observation_lines,
                summary=summary,
                fullLengthRecommended=full_length_recommended,
            ),
            message=self.merchant_voice_service.format_reply_text(
                "Scan suggestions ready to confirm and adjust.",
                voice_config or self._merchant_voice_config(),
    ...
    def _normalize_style_profile_body_type(self, value: Optional[str]) -> Optional[str]:
        normalized = str(value or "").strip().lower().replace(" ", "_")
        allowed = {"hourglass", "pear", "triangle", "rectangle", "apple", "athletic", "other"}
            ...
            response = self.client.responses.parse(
                ...
                input=[
                    ...
                    },
                    {
                        ...
                        "content": json.dumps(
                            {
                                "merchant_context": self._merchant_context(force_refresh=True),
                                "product": {
                                    "title": title,
                                    "category": category,
    ...
    def _system_instructions(self, voice_config=None) -> str:
        return (
            ...
            f" {self.merchant_voice_service.build_voice_system_prompt(voice_config or self._merchant_voice_config(), audience='shopper-facing stylist')}"
        )
    ...
    def _build_prompt_payload(
        ...
        merchant_context = self._merchant_context()
        voice_config = merchant_context.get("chatbot_voice_config") or self._merchant_voice_config()
        payload = {
            "mode": mode,
            "shopper_message": shopper_message,
            "detected_tags": detected_tags,
            ...
            "merchant_voice_style": self.merchant_voice_service.build_style_metadata(voice_config),
            "candidate_products": [
                {
                    "id": item.id,
                    "title": item.title,
    ...
    def _merchant_voice_config(self, force_refresh: bool = False):
        workspace = self.supabase_service.fetch_workspace_snapshot() if force_refresh else None
        if workspace is not None:
            return workspace.chatbot_voice_config
        return self._merchant_context(force_refresh=force_refresh).get("chatbot_voice_config")

    def _merchant_context(self, force_refresh: bool = False) -> dict:
        now = time.time()
        if (
            not force_refresh
            and
        ...
        voice_config = workspace.chatbot_voice_config
        context = {
            ...
            "chatbot_voice_config": voice_config.model_dump(),
            "chatbot_voice_style": self.merchant_voice_service.build_style_metadata(voice_config),
    ...
    def merchant_context(self, force_refresh: bool = False) -> dict:
        return self._merchant_context(force_refresh=force_refresh)
