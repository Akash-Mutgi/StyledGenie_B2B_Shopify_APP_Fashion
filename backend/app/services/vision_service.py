...
from app.models.schemas import AIStackStatus, StyleProfileImageValidation
...
class VisionService:
    ...
    def validate_profile_setup_image(
        self,
        *,
        image_reference: str,
        image_content_base64: Optional[str] = None,
        ...
        if image_content_base64:
            try:
                image_bytes = self._decode_base64(image_content_base64)
            except Exception:
                image_bytes = None
        ...
        width, height = self._extract_image_dimensions(image_bytes, image_mime_type)
        quality_warnings: list[str] = []
        guidance: list[str] = []
        blocking_warnings: list[str] = []

        labels = {str(item or "").strip().lower() for item in (analysis.labels or []) if str(item or "").strip()}
        objects = {str(item or "").strip().lower() for item in (analysis.objects or []) if str(item or "").strip()}
        person_detected = "person" in labels or "person" in objects or analysis.person_count > 0
        full_body_likely_visible = False

        if not person_detected:
            quality_warnings.append("Please include one person clearly in frame.")
        elif analysis.person_count > 1:
            warning = "Multiple people detected, please upload a solo image."
            quality_warnings.append(warning)
            blocking_warnings.append(warning)
        else:
            guidance.append("One person was detected in the frame.")

        if width and height:
            if min(width, height) < 720:
                quality_warnings.append("Image resolution is a little low. A sharper full-body photo will work better.")
            else:
                guidance.append("Image resolution looks strong enough for setup.")

            if height >= int(width * 1.15):
                full_body_likely_visible = True
                guidance.append("Full body likely visible.")
            else:
                warning = "Please retake with your full body visible in frame."
                quality_warnings.append(warning)
                blocking_warnings.append(warning)

        if image_bytes and len(image_bytes) < 80_000:
            quality_warnings.append("Image may be too compressed or blurry. A clearer retake would help.")

        ok = bool(full_body_likely_visible and not blocking_warnings)
        if not guidance:
            guidance.append("Review the photo and edit any profile details manually before saving.")

        if not ok and not quality_warnings:
            quality_warnings.append("Please review the photo before continuing.")

        return StyleProfileImageValidation(
            ok=ok,
            fullBodyLikelyVisible=full_body_likely_visible,
            blurScore=None,
            qualityWarnings=self._dedupe(quality_warnings),
            guidance=self._dedupe(guidance),
            sourceImageUrl=image_reference if self._looks_like_direct_image_url(image_reference) else None,
        )
        ...
        try:
            ...
            payload = {
                ...
                "person_count": sum(
                    1
                    for item in (response.localized_object_annotations or [])
                    if str(getattr(item, "name", "") or "").strip().lower() == "person"
                ),
        ...
        *,
        ...
        payload = {
            ...
            "person_count": sum(
                1
                for item in annotations.get("localizedObjectAnnotations", [])
                if str(item.get("name", "")).strip().lower() == "person"
            ),
