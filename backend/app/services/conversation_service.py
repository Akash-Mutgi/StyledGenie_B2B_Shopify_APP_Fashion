...
class ConversationService:
    ...
    def handle_text_chat(self, payload: ChatRequest) -> ChatResponse:
        session_id = self.supabase_service.ensure_chat_session(payload.customer_id)
        ...
        explicit_segment = self.profile_service.normalize_segment(
            payload.profile_inputs.segment if payload.profile_inputs else None
        )
    ...
    def refine_recommendations(self, payload: RecommendationRefineRequest) -> ChatResponse:
        if payload.swap_category:
            try:
                ...
                return ChatResponse(
                    reply="Sorry, I couldn’t swap that item right now. Please try again.",
    ...
    def _swap_recommendation_response(self, payload: RecommendationRefineRequest) -> ChatResponse:
        ...
        if replacement is None:
            return ChatResponse(
                reply="I couldn’t swap just that piece cleanly from the live catalog, but I can still show another full direction.",
        ...
        if validation_result.status == "fail":
            return ChatResponse(
                reply="I couldn’t swap just that piece cleanly without breaking the look, but I can show another full direction.",
                recommended_products=current_products,
                styling_insights=[],
                image_analysis=payload.image_analysis,
                gap_analysis=payload.gap_analysis,
    ...
    def _refine_image_recommendation_response(self, payload: RecommendationRefineRequest) -> ChatResponse:
        ...
        if validation_result.status == "fail":
            return ChatResponse(
                reply="I couldn’t safely rework that without drifting away from the original image, so I kept the current direction intact.",
                recommended_products=current_products,
                styling_insights=[],
                image_analysis=payload.image_analysis,
                gap_analysis=gap_analysis,
    ...
    def _build_swap_reply(self, mode: str, swap_category: str, replacement_title: str) -> str:
        if mode == "get_inspired":
            return f"I kept the recreated look intact and only swapped the {swap_category} to {replacement_title}."
        if mode == "complete_the_look":
            return f"I kept the rest of the build the same and swapped the {swap_category} to {replacement_title}."
        return f"I kept the outfit direction steady and swapped the {swap_category} to {replacement_title}."
    ...
    def _decision_mode_prompt_response(
        ...
        *,
        ...
        return ChatResponse(
            reply="Do you want a few options, or do you want me to pick the best one for you?",
            recommended_products=[],
            styling_insights=[],
            shopper_profile=shopper_profile,
    ...
    def _compose_image_analysis_reply(self, image_analysis, follow_up: str) -> str:
        ...
        return " ".join(observation_bits + [follow_up]).strip()
    ...
    def _missing_styling_text_prompt(
        ...
        *,
        ...
        if not occasion:
            return (
                "What’s the occasion for this look?",
        ...
        if not weather:
            return (
                "What weather should I style this for?",
        ...
        if (
            ...
            return (
                "Should I keep this more comfortable, sharper, or a little bolder?",
    ...
    def _handle_product_support(
        ...
        *,
        ...
        if product is None:
            reply = "Which product do you mean? Send the product name and I’ll pull the closest match from the catalog."
            payload = SupportPayload(
        ...
        reply = f"{title} is listed under {category}.{price_copy} If you want, I can also help with sizing or send you straight to the product."
        payload = SupportPayload(
            intent="product_question",
            title=title,
    ...
    def _handle_sizing_support(
        ...
        *,
        ...
        if product is None:
            reply = "Tell me the product name first, and I’ll give you a cleaner size recommendation."
            payload = SupportPayload(
        ...
        if missing_fields:
            if "height" in missing_fields:
                reply = "What’s your height? That will help me narrow the size properly."
                requested_fields = ["height"]
            else:
                reply = "Do you want it fitted, regular, or more relaxed?"
    ...
    def _handle_general_support(
        ...
        reply = "Sure — what do you need help with: tracking, returns, delivery, product details, or sizing?"
        payload = SupportPayload(
            intent="general_support",
            title="Support help",
    ...
    def _support_order_lookup_prompt(self, missing_fields: list[str]) -> str:
        if missing_fields == ["order_reference", "email"]:
            return "Can you share both your order number and the email used for the order?"
        if missing_fields == ["order_reference"]:
            return "Can you share your order number?"
        return "Can you share the email used for the order?"
