...
function getPreferredCollageAudience(products = []) {
  ...
  const profileAudience = String(
    shopperProfileDraft.segment || shopperOnboardingData.gender || ""
  ).toLowerCase();
  return ["menswear", "womenswear"].includes(profileAudience) ? profileAudience : "";
}
...
function isCollageAudienceCompatible(product, audience, allowUnknownApparel = false) {
  ...
  if (audience === "menswear") {
    return productAudience === "menswear";
  }
  if (productAudience === audience || productAudience === "neutral") return true;
  if (productAudience && productAudience !== "unknown") return false;

  const kind = getCollageProductKind(product);
  const safelyNeutralKinds = ["jewellery", "earring", "accessory", "bag"];
  return safelyNeutralKinds.includes(kind) || allowUnknownApparel;
}
...
function selectDiverseCollageProducts(products, limit = 5) {
  ...
  const audience = getPreferredCollageAudience(recommended);
...
function showTrackOrderScreen(options = {}) {
  ...
  void sendTextChat("Track my order", {
...
function showReturnItemScreen(options = {}) {
  ...
  void sendTextChat("I need help with a return or exchange", {
    displayText: "Returns or exchanges",
    skipUserEcho: true,
    profileInputs: null,
  });
}
  ...
  groups.forEach((group) => {
    ...
    if (group.title === "Full outfits") {
      const mappedCatalog = catalog
        .filter(isUsableShopifyProduct)
        .map((product) => mapCatalogProductToRecommendation(product));
      const collageLooks = buildShopifyCollageAlternatives(mappedCatalog, 3, { includeBothAudiences: true }).slice(0, 3);
...
async function showShopifyProductsForInspireCategory(categoryLabel) {
  ...
  if (!ranked.length) {
    removeTypingState(typingState);
    addMessage("The live Shopify catalog is temporarily unavailable. Please try this category again.", "bot");
    return;
  }
...
async function renderCreateFullOutfitResult(profileInputs = null, options = {}) {
  ...
  const preferredAudience = ["menswear", "womenswear"].includes(heroAudience)
    ? heroAudience
  ...
  const products = realContextProducts
    .filter((product) =>
      realCatalogProducts.some(
        (candidate) =>
          getCollageProductKind(candidate) === getCollageProductKind(product) &&
  ...
  addButton.addEventListener("click", () => {
    if (shouldUseViewProductInstead(composition)) {
      showProductViewForLocalDemo(composition, context, {
        heading: "Products in this swapped look",
      });
      return;
    }
...
function addLookPreview(products, mode, profile, insights = [], context = null) {
  ...
  const alternatives = context && context.decisionMode
    ? [selectDiverseCollageProducts(products, 7)]
    : buildShopifyCollageAlternatives(products, getLiveOutfitLookCount(products), { includeBothAudiences: true });
  outfitCarouselState = {
    alternatives,
    index: 0,
    profile,
