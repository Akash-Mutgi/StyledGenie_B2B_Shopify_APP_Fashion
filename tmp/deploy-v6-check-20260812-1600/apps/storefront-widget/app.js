...
function showOutfitZoomScreen(products) {
  ...
  overlay.setAttribute("aria-label", "Outfit details");
  ...
  title.textContent = "Outfit details";
  ...
  hint.className = "outfit-zoom-hint";
  hint.textContent = "Tap an item to view the product";

  const productGrid = document.createElement("div");
  productGrid.className = "outfit-zoom-products";
  safeProducts.forEach((product) => {
    const link = document.createElement(product.product_url ? "a" : "div");
    link.className = "outfit-zoom-product";
    ...
    name.textContent = product.title || product.category || "Product";
  ...
  overlay.append(header, visual, hint, productGrid);
...
const CARE_MENU_ACTIONS = [
  ...
  { label: "Shipping information", icon: "track", flow: "shipping" },
  { label: "FAQ's", icon: "faq", flow: "faq" },
  { label: "Talk to agent", icon: "agent", flow: "agent" },
  { label: "Raise support ticket", icon: "agent", flow: "ticket" },
];
...
function showAgentConnectingScreen(topic) {
  ...
  window.setTimeout(() => {
    addMessage("Sara has joined the chat", "bot");
...
function showReturnItemScreen(options = {}) {
  ...
  void renderCareShopifyProductPicker((selected) => {
    addMessage(`Return or exchange ${selected.title}`, "user");
...
async function renderCompleteDemoResults(selectedCategories = []) {
  ...
  const categoryGroups = [
    ...
    { title: "Jewellery", pattern: /jewel|necklace|earring|bracelet|watch|accessor|belt|chain|ring/i },
  ...
  if (visibleProducts.length) {
    ...
    actions.className = "flow-action-row flow-action-row--three";
    [
      ["Save outfit", () => addMessage("Outfit saved.", "bot")],
      ["Add to wishlist", () => addMessage("Added to your wishlist.", "bot")],
      ["Add to cart", (button) => void addProductsToCartBulk(visibleProducts, button)],
    ].forEach(([label, handler]) => {
      const button = document.createElement("button");
      button.type = "button";
      button.className = label === "Add to cart" ? "flow-primary-button" : "flow-secondary-button";
      button.textContent = label;
...
async function showShopifyProductsForInspireCategory(categoryLabel) {
  ...
  addMessage(`Here are Shopify picks for ${categoryLabel.toLowerCase()}. Select one and I'll build the look around it.`, "bot");
  addSuggestionChips(
    ranked.map((product) => mapCatalogProductToInspireOption(product)),
    (option) => {
  ...
  actions.className = "inspire-social-actions figma-card-screen";
  ["Like", "Save", "Share"].forEach((label) => {
    const button = document.createElement("button");
    button.type = "button";
    button.className = "flow-secondary-button";
    button.textContent = label;
    button.addEventListener("click", async () => {
      if (label === "Share" && navigator.share) {
        try {
          await navigator.share({ title: categoryLabel, text: `StyledGenie ${categoryLabel} picks` });
          return;
        } catch (error) {
          /* Dismissing the native share sheet keeps the current flow unchanged. */
        }
      }
      addMessage(label === "Like" ? "Look liked." : label === "Save" ? "Look saved." : "Share link ready.", "bot");
    });
    actions.appendChild(button);
  });
...
async function renderCreateFullOutfitResult(profileInputs = null, options = {}) {
  ...
  const products = selectDiverseCollageProducts(shopifyProducts, 5).map((product) => ({
    ...mapCatalogProductToRecommendation(product),
...
function setOutfitSwapHeaderActive(context = null) {
  if (widgetWelcomeTitle) {
    widgetWelcomeTitle.textContent =
      context && context.uiMode === "inspire" ? featureLabels.inspire : featureLabels.complete;
  }
  if (widgetPanel) {
